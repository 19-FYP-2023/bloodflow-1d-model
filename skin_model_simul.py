...
script_usage_msg = f"[Usage] To run for a specific patient folder in the {OUTPUTS_FOLDER_PATH}/ directory:\n  python skin_model_simul.py <patient data folder name>\n"
script_usage_msg += f"[Usage] To run for all patient folders in the {OUTPUTS_FOLDER_PATH}/ directory:\n  python skin_model_simul.py\n"

if len(sys.argv) != 2 and len(sys.argv) != 1:
    raise Exception(f"[ERROR] Incorrect invocation of the script\n{script_usage_msg}")

patient_folders = []

if len(sys.argv) == 2:
    patient_folders.append(sys.argv[1])
else:
    for name in os.listdir(OUTPUTS_FOLDER_PATH):
        if os.path.isdir(os.path.join(OUTPUTS_FOLDER_PATH, name)) and "patient" in name:
            patient_folders.append(name)

if len(patient_folders) == 0:
    raise Exception(f"[ERROR] No patient folders found inside the {OUTPUTS_FOLDER_PATH}/ directory")

# making a folder with the name of the patient if not available
for patient_folder_name in patient_folders:
    try:
        base = f"{OUTPUTS_FOLDER_PATH}/{patient_folder_name}/"
        result_folder = f"{RESULTS_FOLDER_PATH}/{patient_folder_name}/"
